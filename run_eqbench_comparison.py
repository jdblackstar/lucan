...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run EQBench comparison between Lucan and Claude"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug output")
    parser.add_argument(
        "--persona", default="lucan", help="Persona to use for Lucan (default: lucan)"
    )
    parser.add_argument(
        "--claude-model", default="claude-sonnet-4-20250514", help="Claude model to use"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("eqbench_results"),
    ...
    try:
        ...
        lucan_result, claude_result = await tester.run_comparison(
            lucan_chat, args.claude_model
        )
        ...
        if lucan_result.total_score > claude_result.total_score:
            print(
                f"Winner: Lucan (+{lucan_result.total_score - claude_result.total_score:.2f} points)"
            )
        else:
            print(
                f"Winner: Claude (+{claude_result.total_score - lucan_result.total_score:.2f} points)"
            )
